...
export function StocksTab() {
  ...
  useEffect(() => { if (stocks.length === 0) fetchStocks() }, [])
      ...
      <div className="grid grid-cols-[1fr_auto_auto_auto] gap-2 px-2 mb-3">
        {['Symbol', 'Price', 'Chg%', ''].map((h, i) => (
          <span key={h || i}
            ...
            style={{ color: 'var(--col-text-3)', textAlign: i === 3 ? 'center' : i > 0 ? 'right' : 'left', width: i === 1 ? 72 : i === 2 ? 56 : i === 3 ? 28 : 'auto' }}>
            {h}
          </span>
        ))}
      </div>
          ...
          {stocks.map((stock, i) => {
            ...
            return (
              ...
              >
                ...
                <motion.button
                  whileTap={{ scale: 0.80 }}
                  onClick={(e) => { e.stopPropagation(); sounds.tap?.(); toggleWatchlist(stock.symbol) }}
