 ...
 * CSuitePanel.jsx
 * 
 * A bottom-sheet modal where players can hire executive managers (CEO, CFO, etc.) 
 * to gain passive boosts (revenue multipliers, reduced upgrade costs).
 * 
 * Features a tabbed interface and visual distinction between manager rarities.
 */
...
// For now, it serves as the master list of available executives.
const MOCK_MANAGERS = [
...
/**
 * Individual Manager Card Component
 */
function ManagerCard({ manager, hired, onHire, onFire }) {
  ...
  return (
    <motion.div
      layout // Allows Framer Motion to smoothly animate list reordering
      className="rounded-2xl overflow-hidden p-4"
      style={{
        // If hired, the card lights up with the manager's theme color
        background: isHired
    ...
    >
      ...
      {/* Action Area (Salary + Hire/Fire Button) */}
      <div className="flex items-center justify-between">
        ...
        {!isHired ? (
          <motion.button whileTap={{ scale: 0.95, y: 1 }} onClick={() => onHire(manager)}
            className="px-4 py-2 rounded-xl text-xs font-black min-h-[40px] flex items-center justify-center"
            style={{
...
export default function CSuitePanel({ open, businessId, onClose }) {
  const [managers, setManagers] = useState(MOCK_MANAGERS)
  const [hired, setHired] = useState({})  // Map of role -> manager object (e.g. { ceo: {id: 1...} })
  ...
  const handleHire = (manager) => {
    // Prevent hiring multiple people for the same role
  ...
  // Only show managers for the currently selected tab
    ...
    <AnimatePresence>
      ...
      <motion.div
        ...
        style={{
          ...
          /* Use dvh so it respects the soft keyboard on mobile */
          maxHeight: '88dvh',
          maxHeight: '88vh',
        }}
      >
        ...
        <div className="px-4 pb-safe pb-6 overflow-y-auto flex-1 flex flex-col min-h-0">
          {/* Header */}
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={onClose} className="w-10 h-10 rounded-xl flex items-center justify-center"
              ...
              <X size={18} style={{ color: 'var(--col-text-2)' }} />
            </button>
          </div>

          {/* ── Active C-Suite Overview ── */}
          {/* Shows a quick row of currently hired managers across all roles */}
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 mb-4 scrollbar-hide">
            {roles.map(r => (
              <button key={r} onClick={() => setTab(r)}
                ...
                {r} {hired[r] ? '✓' : ''} {/* Show checkmark if slot is filled */}
              </button>
            ))}
          </div>

          {/* ── Manager List ── */}
          {/* overscroll-contain prevents pull-to-refresh from firing inside the sheet */}
          <div className="space-y-3 overflow-y-auto overscroll-contain" style={{ maxHeight: '45vh', maxHeight: '45dvh' }}>
            {filtered.length === 0 ? (
